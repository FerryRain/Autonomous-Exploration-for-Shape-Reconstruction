...
#
# v_points = np.asarray(pcd.points)*10 # scale if needed.
# v_normals = np.asarray(pcd.normals)

# points = np.load(f"D:\Code\AMMH\Env\Results\\ablation\Real_wo_dummy_global_sphere_10_09_21_22\explored_x_110.npy")
# val = np.load(f"D:\Code\AMMH\Env\Results\\ablation\Real_wo_dummy_global_sphere_10_09_21_22\explored_y_110.npy")
...
for i in range(training_iter):
    ...
    optimizer.step()
print(f"training takes: {time.time() - star:}")
...
print(f"Interface Time: {time.time()-_}")
...
estimated_surface = estimated_surface[estimated_surface[:, -1] <=4]
estimated_surface = estimated_surface[estimated_surface[:, -1] >=-4]
