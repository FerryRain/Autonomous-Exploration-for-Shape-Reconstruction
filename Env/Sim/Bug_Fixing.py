...
def move_to_new(robot, controller, quat, gpis, hand_pos_now, entities, contact_force, alpha_base=0.05, beta_base=0.06):
...
def run_simulator(sim: sim_utils.SimulationContext, entities):
    ...
    temp_min = np.array([-0.16, -0.16, -0.16])
    temp_max = np.array([0.16, 0.16, 0.16])
    ...
    K_p_rot, K_i_rot, K_d_rot = 0.01, 0.0, 0.01  # rot p i d of  PID
    dt = 0.01
