...
def run_exploration(robot: Real_Robot):
    ...
    er_gpis = init_normal_HER_GPIS_uncertainty(temp_min, temp_max, points_num, intervals, res=gpis_res,
    ...
    er_gpis.init_gpis(f / f_norm, torch.tensor(arm.get_ee_position()[:3] - origin_pos))

    normals_buf = torch.empty((0, 3), device="cuda")

    # Simulate physics
    while True and er_gpis.time_step <= 120:
        if get_exploration_status_real(count, 25):
            er_gpis = update_gpis_uncertainty_mm(arm, explored_queue, touched_buf, untouched_buf, er_gpis, normals_buf)
            touched_buf = torch.empty((0, 3), device="cuda")
            untouched_buf = torch.empty((0, 3), device="cuda")
            normals_buf = torch.empty((0, 3), device="cuda")
            count = 0

        generate_next_point_limited_z_force_normal_mm(robot, arm, sensor, origin_pos.copy(), er_gpis,
        ...
        if er_gpis.time_step == 50  or er_gpis.time_step == 70:
            direction_xy = er_gpis.get_max_uncertainty_xy_direction(arm.get_ee_position()[:3] - origin_pos, z_min=-3, z_max=3)
            step = 0
            print("Global exploration")
            while True:
                f = np.array(sensor.read_force_data()[:3])
                f_norm = np.linalg.norm(f)
                if f_norm > force_threshold and step >= 10:
            ...
            save_path = os.path.join(er_gpis.store_path, f"global_buffer_{er_gpis.time_step}.npy")
            np.save(save_path, global_buffer.cpu().numpy())
            # egpis.time_step += 1

            print("--------------------------------------------------------")
            print(f"saved explored_x_{er_gpis.time_step}.npy to", save_path)

        if er_gpis.time_step % 10 == 0 and er_gpis.time_step >= 70:
            direction_xy = er_gpis.get_max_uncertainty_xy_direction(arm.get_ee_position()[:3] - origin_pos, z_min=-3,
                                                                  z_max=3)
            step = 0
            print("Global exploration")
            while True:
        ...
        # if er_gpis.time_step % 10 == 0:
        #     er_gpis.down_sample_points(r=1)
        #     er_gpis.init_gpis(f, torch.tensor(arm.get_ee_position()[:3] - origin_pos))
        # time +1
        count += 1
